...
{
    let d = document, a = d.compatMode == "BackCompat" ? d.body : d.documentElement;
...
{
    const userAgent = window.navigator.userAgent const isExplorer = (exp) => {
        return userAgent.indexOf(exp) > - 1 
    }
    if (isExplorer('MSIE'))
    {
        return 'IE' else if (isExplorer('Firefox')) return 'Firefox' else if (isExplorer('Chrome')) return 'Chrome' else if (isExplorer('Opera')) return 'Opera' else if (isExplorer('Safari')) return 'Safari';
    }
}
/**
 * 禁止鼠标右键、选择、复制
 ...
 * @returns {Promise<any>}
 */
export function fileToBase64String(file, format = ['jpg', 'jpeg', 'png', 'gif'], size = 20 * 1024 * 1024, 
formatMsg = '文件格式不正确', sizeMsg = '文件大小超出限制') 
{
    return new Promise((resolve, reject) => 
    {
        ...
        fileReader.onload = () => 
        {
            ...
            resolve({
                base64String : res, suffix : suffix
            });
            reject('异常文件，请重新选择');
        }
    })
...
{
    let arr = base64.split(','), mime = arr[0].match(/:(.*?);/)[1], bstr = atob(arr[1]), n = bstr.length, 
...
export const createTree = (items, id = null, link = 'pid') => 
{
    items.filter(item => item[link] === id).map(item => ({
        ...item, children : createTree(items, item.id) 
    }));
};
/**
...
export function type(target) 
{
    ...
    let template = 
    {
        "[object Array]" : "array", "[object Object]" : "object", "[object Number]" : "number - object", 
        "[object Boolean]" : "boolean - object", "[object String]" : 'string-object' 
    };
    if (target === null) {
        return 'null';
    }
...
export function changeToChinese (Num) 
{
    ...
    for (var i = part[0].length - 1; i >= 0; i--) 
    {
        ...
        var tmpnewchar = "" var perchar = part[0].charAt(i);
        switch (perchar) 
        {
            case "0":
    ...
    if (Num.indexOf(".") != - 1) 
    {
        ...
        {
            tmpnewchar = "" perchar = part[1].charAt(i) switch (perchar) 
            {
                case "0":
                tmpnewchar = "零" + tmpnewchar;
                break;
 ...
 * 去除字符串中的html代码
 * @param { str } String  
 * console.log(removeHTML('<h1>哈哈哈哈<呵呵呵</h1>')) // 哈哈哈哈<呵呵呵前世孽缘今生话
 */
export const removeHTML = (str = '') => str.replace(/<[\/\!]*[^<>]*>/ig, '');
...
export const formatPhone =  (phone)=> phone.replace(/(\d{3})\d{4}(\d{4})/, "$1****$2");
   ...
   * 数组去重
   * @param arr{Array}: 数组
   * @param u_key{string}: 属性值
   * let arrayList = [{id:'1',name:'one'},{id:'2',name:'tow',},{id:'3',name:'three'},{id:'1',name:'one'},{id:2,name:'tow',},{id:'3',name:'three'}]
   *  unique(arrayList,'id')
   */
